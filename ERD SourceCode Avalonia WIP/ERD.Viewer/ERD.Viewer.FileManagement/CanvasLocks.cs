    ...
    public sealed class CanvasLocks
    {
        ...
        public void UnlockFile(string modelSegmentControlName)
        {
            ...
            try
            {
                ...
                lock (lockMethodObject)
                {
                    ...
                    fileText.Append(File.ReadAllText(LockFullFileName));
        ...
        public void RemoveUserFileLockObject()
        {
            try
            {
                ...
                {
                    Paths.WaitFileRelease(FileLockName);

                    string[] fileText = File.ReadAllLines(FileLockName);
        ...
        private void WaitExistsLockRelease()
        {
            ...
            try
            {
                while (File.Exists(FileLockName))
                {
                    Sleep.ThreadWait(300);
                }

                File.WriteAllText(FileLockName, Environment.UserName);
            }
            catch
            {
                goto REWAIT;
