using ERD.Viewer.Tools.Reflective;
...
namespace ERD.Viewer.Models
{
    ...
    public class ModelBase : INotifyPropertyChanged
    {
        ...
        public Dictionary<string, Tuple<int, object, object>[]> Changes
        {
            get
            {
                return changesList.GroupBy(g => g.Item1)
        ...
        public Tuple<object, object> this[string propertyName]
        {
            get
            {
                Tuple<string, int, object, object>[] resultPack = changesList.Where(k => k.Item1 == propertyName).ToArray();
        ...
        protected virtual void OnPropertyChangedStringCompair<T>(string name, ref T oldValue, T newValue)
        {
            ...
            if (oldValue == null || (newValue != null && oldValue.ToString() != newValue.ToString()))
            {
                if (oldValue != null)
                {
                    AddChangs(name, oldValue, newValue);
                }
        ...
        protected virtual void OnPropertyChangedStringCompair<T>(string name, ref T? oldValue, T? newValue) where T : struct
        {
            if (oldValue == null && newValue == null)
            {
                return;
        ...
        protected virtual void OnPropertyChanged<T>(string name, ref T? oldValue, T? newValue) where T : struct, IComparable<T>
        {
            if (oldValue == null && newValue == null)
            {
                return;
            ...
            if (oldValue != null && (oldValue.Equals(newValue) || Comparer<T?>.Default.Compare(oldValue, newValue) == 0))
            {
                return;
            }
        ...
        {
            int indexCount = changesList.Count;

            changesList.Add(new Tuple<string, int, object, object>(property, indexCount, oldValue, newValue));
        }
    }
}
