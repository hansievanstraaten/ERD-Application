    ...
    public class DatabaseCompare
    {
        ...
        private void CheckRelations()
        {
            foreach (TableModel fromDatabase in this.databaseTables.Where(fk => fk.Columns.Any(col => col.IsForeignkey)))
            {
                ...
                Dictionary<string, List<ForeignKeyObjectModel>> databaseForeignkeyStructure = fromDatabase.Columns
                  .Where(dk => dk.IsForeignkey)
                  .SelectMany(col => col.ForeignKeys)
                  .Where(g => g.IsVertualRelation == false)
                  .GroupBy(dg => dg.ForeignKeyTable.ToUpper())
                  .ToDictionary(dd => dd.Key, dd => dd.ToList());

                foreach (KeyValuePair<string, List<ForeignKeyObjectModel>> keyPair in databaseForeignkeyStructure)
                {
                    ...
                    foreach (ForeignKeyObjectModel databaseColumn in keyPair.Value)
                    {
                        ColumnObjectModel canvasColumn = canvasKeyTable.Columns.FirstOrDefault(cc => cc.ColumnName.ToUpper() == databaseColumn.LocalColumnName.ToUpper());
                        ...
                        if (canvasColumn.ForeignKeys.Any(ck => ck.ForeignKeyColumn.ToUpper() == databaseColumn.ForeignKeyColumn.ToUpper()) == false)
                        {
                            this.result.Add(new CompareResultModel
                            {
                                TableObject = fromDatabase,
                                ObjectName = databaseColumn.ForeignConstraintName,
                                Message = $"Foreign Key Constraint Exist in Database Model but not on ERD Model. ({databaseColumn.LocalColumnName})",
            ...
            foreach (TableModel fromCanvas in this.canvasTables.Where(fk => fk.Columns.Any(col => col.IsForeignkey)))
            {
                EventParser.ParseMessage(this, dispatcher, fromCanvas.TableName, $"Compare Table {fromCanvas.TableName} Foreign Constraints");

                Dictionary<string, List<ForeignKeyObjectModel>> canvasForeignkeyStructure = fromCanvas.Columns
                  .Where(dk => dk.IsForeignkey)
                  .SelectMany(col => col.ForeignKeys)
                  .Where(g => g.IsVertualRelation == false)
                  .GroupBy(dg => dg.ForeignKeyTable.ToUpper())
                ...
                foreach (KeyValuePair<string, List<ForeignKeyObjectModel>> keyPair in canvasForeignkeyStructure)
                {
                    ...
                    foreach (ForeignKeyObjectModel databaseColumn in keyPair.Value)
                    {
                        ...
                        if (canvasColumn.ForeignKeys.Any(ck => ck.ForeignKeyColumn.ToUpper() == databaseColumn.ForeignKeyColumn.ToUpper()) == false)
                        {
                            this.result.Add(new CompareResultModel
                            {
                                TableObject = fromCanvas,
                                ObjectName = databaseColumn.ForeignConstraintName,
                                Message = $"Foreign Key Constraint Exist on ERD Model but not in Database. ({databaseColumn.LocalColumnName})",
                                ObjectType = ObjectTypeEnum.ForeignKeyConstraint,
                                ObjectAction = ObjectActionEnum.CreateInDatabase
                            });
                        }
